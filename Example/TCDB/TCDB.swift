...
open class TCDB: NSObject {
    ...
    ///   - onqueue: 线程
    ///   - callback: 回调
    class func execSql( _ method : EXECSQLMethod, sql : SQL , onqueue : DispatchQueue = dbQueue ,  callback:@escaping ((Any)->()) ){
        // 先把OC字符串转化为C字符串
    ...
    /*
     1 一个唯一的对列名
     */
    static private let dbQueue = DispatchQueue(label: "com.tanchao.dataBase")
    
    /// 打开数据库
    ///
    /// - Parameter DBName: 数据库名称
    @discardableResult
    class func openDB(DBName: String) -> Bool{
